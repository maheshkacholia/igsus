 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class ManageIndicatorsScreen extends JFrame implements ManageIndicatorInterface,ListSelectionListener, KeyListener, ActionListener, MouseListener
{
	public JTextField stockcodeText;
	public Vector indicatorNameList=null;
	/**
	...
	public ManageIndicatorsScreen(PanelForGraph panelforgraph)
	{
		...
		this.setTitle("Manage Indicators");
		this.move(300, 100);
	...
	private javax.swing.JPanel getJContentPane()
	{
		...
		IGSLabel lable1 = new IGSLabel("Select Indicator:");
		lable1.setBounds(xinc, yinc, 100, 15);
		jContentPane.add(lable1, null);

		xinc = xinc + 400;

		IGSLabel lable2 = new IGSLabel("Selected Indicator:");
		...
		indicatorNameList = ManageIndicator.getIndicatorName();
		list = new JList(indicatorNameList);
		...
		selectedIndicators = panelforgraph.macdgraph.getIndicators();
		
		list1 = new JList(selectedIndicators);
...
//		comment.disable();
	...
	public void actionPerformed(ActionEvent e)
	{
		 ...
		 if (o instanceof JButton)
		{
			...
			if (name.equalsIgnoreCase("select"))
			{
				...
				if(value == -1)
					return;
				String indname	=	ManageIndicator.indicatorVector.elementAt(value)+"";
				
				if(!ManageIndicator.isCustom(indname))
				{
					Indicator selInd = ManageIndicator.getIndicatorObject(indname);
					TechIndicatorEntryPanelNew t = new TechIndicatorEntryPanelNew(selInd,this);
				//	System.out.println(selInd.indicatorName);
					
				}
				else
				{
					//String filename =  ManageIndicator.returnfileName(indname);
					panelforgraph.applyCustmizedIndicator(this,StockConstants.NEW_IND_DIR, indname+".fl");
			...
			else if (name.equalsIgnoreCase("deselect"))
			{
				...
				Indicator sel = (Indicator)panelforgraph.macdgraph.indHash.get(value.toString());
				if(sel != null)
					panelforgraph.macdgraph.removeIndicator(sel);
				selectedIndicators = panelforgraph.macdgraph.getIndicators();
				list1.setListData(selectedIndicators);						
			} 
	...
	public String getComments(String indname)
	{
		if(!ManageIndicator.isCustom(indname))
		{
			int id = ManageIndicator.getIndiPosition(indname);
			if(id==-1)
				return "";
			else
			{
				try
				{
					return ManageIndicator.indidescr[id];
				}
				catch(Exception e)
				{
					return "";
		...
		try
		{
			String s = Utility.getFileContent(StockConstants.NEW_IND_DIR+ "/"+indname+".fl");
	...
	{
		ManageIndicatorsScreen addremove = new ManageIndicatorsScreen(null);
	}
	/* (non-Javadoc)
...
//	@Override
	public void addIndicator(Indicator selInd) 
	{
		...
		// TODO Auto-generated method stub
		panelforgraph.addIndicator(selInd);
		selectedIndicators = panelforgraph.macdgraph.getIndicators();
		list1.setListData(selectedIndicators);						
...
//	@Override
	public int getIndiactorType() {
		// TODO Auto-generated method stub
		return ManageIndicatorInterface.INDICATOR;
	}
