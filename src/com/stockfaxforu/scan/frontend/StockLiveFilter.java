...
import com.stockfaxforu.livedata.ShowLiveMarketInterface;
 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class StockLiveFilter extends JFrame implements StockFilterInterface,FrameDataInterface,ActionListener, ListSelectionListener, WindowListener,MouseListener
{
	/**
	...
	int xmax=1100;
	int ymax=900;
	ShowLiveMarketInterface alertscreen;
	public StockLiveFilter()
	{
		super();
		initialize();
	}
	
	public StockLiveFilter(ShowLiveMarketInterface alertscreen,int xmax,int ymax)
	{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if (obj instanceof JButton)
		{
			...
			if (name.equals("selectformula"))
			{
				...
				fd.setFile("*.igsql");
				fd.setDirectory(StockConstants.SQL_DIR_INTRA);
...
//				dispose();
		...
		//			e1.printStackTrace();
			...
			//	while(true)
			//	{
					Runnable r = new LiveScannerThread(this);
...
//		output = master.getAllStocksMultipleQueryHash(symbolVector, query);
		...
		output = buysell.getAllStocksQueryIntraday(alertscreen.getLiveDataHashMap(), query);		
...
//				String[] temp = ScanUtility.getSymbolDetail(s,outputField);
	...
	{
		JFrame f = new JFrame();
		StockLiveFilter newscan = new StockLiveFilter(null,1000,1000);
