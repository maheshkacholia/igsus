 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class AddRemoveStock extends JFrame implements ListSelectionListener, KeyListener, ActionListener, MouseListener
{
	...
	public AddRemoveStock()
	{
		...
		this.setTitle("Add/Remove Stock From Category");
		this.move(300, 100);
		this.setSize(850, 525);
		this.setVisible(true);
	}
	public AddRemoveStock(String category)
	{
		//		super(null);
		setBackground(Color.lightGray);
		...
		this.setContentPane(getJContentPane());
		this.setTitle("Add Stock to Category");
		this.move(300, 100);
		this.setSize(850, 525);
		this.setVisible(true);
	}
...
//			ComboBoxRendererSymbol custom = new ComboBoxRendererSymbol(v);
			...
			indexes.setName("indexes");
//			indexes.setRenderer(custom);
			jContentPane.add(indexes, null);

		}
		else
		{
			IGSLabel lable = new IGSLabel("Category: "+category);
//			lable.setFont(new Font("Arial", Font.PLAIN, 12));
//			lable.setForeground(Color.black);
			lable.setBounds(xinc, yinc, 300, 15);
			jContentPane.add(lable, null);
		...
		xinc = 50;	
		IGSLabel lable10 = new IGSLabel("Enter Stock Code:");
//		lable10.setFont(new Font("Arial", Font.PLAIN, 12));
//		lable10.setForeground(Color.black);
		lable10.setBounds(xinc, yinc, 200, 15);
		jContentPane.add(lable10, null);

		yinc = yinc + 20;
		xinc = 50;	
		stockcodeText = new JTextField();			
		stockcodeText.setBounds(xinc, yinc, 150, 20);
		jContentPane.add(stockcodeText, null);
		stockcodeText.setName("stockcodeText");
		stockcodeText.addActionListener(this);
		stockcodeText.addKeyListener(this);
	
		addstock = new JButton("Add to List");
		addstock.setBounds(xinc + 175, yinc, 100, 20);
		jContentPane.add(addstock, null);
		addstock.setName("addstock");
		addstock.addActionListener(this);
		addstock.addKeyListener(this);
	
		
		yinc = yinc + 25;
		xinc = 50;	

		IGSLabel lable1 = new IGSLabel("Select Stcok:");
		lable1.setBounds(xinc, yinc, 100, 15);
		jContentPane.add(lable1, null);

		xinc = xinc + 300;

		IGSLabel lable2 = new IGSLabel("Selected Stcok:");
		...
		stockCode = Utility.getStockName();
		list = new JList(stockCode);
		list.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
		list.addListSelectionListener(this);
		JScrollPane listScrollPane = new JScrollPane(list);
		listScrollPane.setBounds(xinc, yinc, 325, 300);
		...
		pictureScrollPane.setBounds(xinc + 425, yinc, 325, 300);
		...
		deselect.setBounds(xinc , yinc, 75, 20);
...
//			list1.
		...
		else if (o instanceof JButton)
		{
			...
			if (name.equalsIgnoreCase("select"))
			{
				...
				{
					String stockCode = Utility.getStockNameFromDes(value[i]+"");
					if(!selectedStock.contains(stockCode))
						selectedStock.addElement(stockCode);
				}
				list1.setListData(selectedStock);						
	...
	public void saveIndex()
	{
		...
		if(category==null)
		{
			indexStr=(String)indexes.getSelectedItem();
		...
		{
			indexStr=category;
			IndexUtility.apendNewStocksToCategory(selectedStock, indexStr);
